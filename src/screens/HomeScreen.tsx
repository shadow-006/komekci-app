...
export function HomeScreen() {
  ...
  const todayTasks = useMemo(
    () => tasks.filter((task) => task.date === today).sort((a, b) => a.createdAt - b.createdAt),
    [tasks, today]
  );
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.statsRow}>
        <Card style={styles.statCard}>
          <Text style={styles.statLabel}>💧 {t.home.water}</Text>
          ...
          <Text style={styles.statGoal}>
            {t.home.goal}: {(goals.waterGoalMl / 1000).toFixed(1)} L
          </Text>
        </Card>
        <Card style={styles.statCard}>
          <Text style={styles.statLabel}>🔥 {t.home.calorie}</Text>
          <Text style={styles.statValue}>{todayLog.calories} kcal</Text>
          <ProgressBar value={caloriePct} complete={caloriePct >= 1} />
          <Text style={styles.statGoal}>
            {t.home.goal}: {goals.calorieGoalKcal} kcal
          </Text>
        </Card>
      </View>

      <Card style={{ marginTop: spacing.lg }}>
        <SectionHeader title={`${t.home.todayTasks} (${remaining} ${t.home.remaining})`} />
        ...
        {todayTasks.length === 0 ? (
          <Text style={styles.empty}>{t.home.noTasks}</Text>
        ) : (
          todayTasks.map((task) => <TaskItem key={task.id} task={task} onToggle={toggleTask} onRemove={removeTask} />)
        )}
      </Card>
    </ScrollView>
  );
